...
- **SYNTHESIZE**（核心）: 全草案を読み、**否定・保存・高次化**の三契機で止揚する昇華推理（temp 0.9）→ 止揚推理だけを読む最終化（temp 0.0）。単一観点の草案にはない超越的な統合解を構成する。
  ...
  - 「昇華が単発生成を上回る」は**設計目標であり、`compare` による実測で検証する**。検証の経路は [昇華優位性の計測](#昇華優位性の計測compare) を参照。
...
- **昇華優位性は実測で検証する**: 「昇華が単発生成を上回る」は設計目標であり、`compare --runs N` の
  実測データで検証する。タスクやモデルによっては昇華がベースラインを下回ることもあり、
