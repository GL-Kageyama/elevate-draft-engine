...
（エージェントファイルの「草案の作り方」に組込み済み）。昇華（Aufheben）が各草案の最強の主張を
推測に頼らず確実に拾い、矛盾検出のモデル依存を下げる。本編の自由な論述・声・多様性は
そのまま保たれ、枠は断言に絞る。「反論されそうな点」は**付けない**——草案に弱点を
先読みさせると自由な逸脱が萎縮するため、反論の検出は昇華段階の Aufheber が引き受ける。
...
| `--no-strong-claim` | エージェントから「最強の主張」断言枠を除去。枠あり/なしの昇華品質差を測定するアブレーション |
