...
**対処（2026-08-09）**: 分野の数だけフォーマットを事前定義するのでなく、パイプライン開始前に
**タスクから期待される出力形式を LLM が動的に抽出**する（`extract_format`。1回の軽量コール、温度0.0、同一タスクはキャッシュ）。
...
**対処（2026-08-09）**: 起動時に前提知識の有無を指定し、ある場合はそれを生成の土台として
全段階に注入する。fmt（形の制約）と対になる**内容の制約**であり、直交する:
