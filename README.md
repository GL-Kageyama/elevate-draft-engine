...
.venv/bin/python -m pytest tests/ -q     # 90件
...
統合版（`elevated.md`）を両方保存する。それらを客観視するための比較ドキュメントを生成する:
...
統合優位性が特定のタスク・構成に依存しないことを、複数条件の実測で示すのが目的である。
実測の正体は examples/ の各サンプルを参照（n=1 の knowledge-search は統計的な優位性を示せておらず、
差はノイズの範囲内であることを明示する）。

### 反復改善（improve）— 統合版を磨くループ

`improve` は、一度作った統合版を**繰り返し磨き上げる**ためのループである:

```
統合版 → 改修の草案(複数) → 統合 → 新しい統合版 → （繰り返し）
```

- round 1: オリジナルタスクから発散 → 統合で**初回の統合版**を作る。
- round 2 以降: 各エージェントが**前回の統合版を改修した草案**を書き、それらを統合して
  次の統合版を作る。統合版の成果がループを回すごとに相続され、積み上がっていく。
  （「統合したらおしまい」ではなく、統合版を土台にしてさらに磨く。）
- 各 round は `round_NN/` に分離保存（`draft_*` / `reconciliation` / `elevated`）され、
  履歴が追える。`progress.md` に全 round の長さと評価が記録される。

```bash
python main.py improve "タスク" --rounds 3                # 3回の統合を繰り返す
python main.py improve "タスク" --rounds 5 --evaluate     # 各ラウンドを5軸評価
```

| オプション | 動作 |
|---|---|
| `--rounds N` | 統合を繰り返す回数（既定 3） |
| `--evaluate` | 各ラウンドの統合版を5軸評価し、overall を progress.md に記録 |
...
```
[round 1 統合版] overall=0.600（Revise）   ← 素の生成相当。天井でなく改善の余地がある
[round 2 統合版] overall=0.720（Pass）   +0.120
[round 3 統合版] overall=0.720（Pass）   +0.000 → 頭打ちと判断し停止（過修正を避ける）
```
...
`--evaluate` の採点は、**このエンジンが何をする機械か**（ポリシー）を測る5軸で行う。

> 複数の独立した視点を統合して、単一視点を超える成果物を生む —— そのポリシーを評価の基準にした。

| # | 軸 | 評価の問い | ポリシー | 高評価（0.7〜0.8） | 低評価（0.0〜0.4） |
|---|---|---|---|---|---|
| 1 | **多様性**（視点の横断） | 複数の世界を見ているか | 発散 | 複数の視点・価値観を横断し、分野を広く賄う | 単一の視点に閉じている |
| 2 | **統合性**（矛盾解決・連結） | 併記でなく噛み合っているか | 統合（併記・平均化は不合格） | 視点間の矛盾を解消し、相互に連結した高次構造 | 断片の寄せ集め |
| 3 | **超越性**（第3の位置） | どの単一視点にもない解か | 超越（統合を経て初めての新観点） | 統合を経て初めて得られる新たな視点が明確 | 焼き直し |
| 4 | **誠実性**（確信と不確実の区別） | 断定と推測を区別しているか | 誠実さ（未実証の明記） | 不確実な点は条件付き・前提として明示 | 不確実なことを断定 |
| 5 | **実用性**（具体・実行可能） | 誰がどう使えるか | 実行性（具体的・整合的・実行可能） | 実行の筋道と利用者が具体的に描ける | 抽象的で実行可能性が確認できない |

- **重みは均等（各 0.20）**: `overall = 多様性×0.20 + 統合性×0.20 + 超越性×0.20 + 誠実性×0.20 + 実用性×0.20`
- **全軸「高いほど良い」**: 旧リスク認識（Risk）軸は「正直なリスク開示ほど評価が下がる」構造的バイアスのため廃止し、反転ロジック（1−risk）も撤廃した。
- **採点アンカー**: 0.5 = 無難 / 0.7〜0.8 = 確かに良い / 0.9 以上 = 凡庸な生成にはない固有の枠組み・見方の転換。凡庸な出力を 0.7 以上にしない。
- **盲検**: 評価は生成と独立した系統（評価専用モデル）で、成果物の出所・生成方法を知らされない。

## 知恵の評議会による評価

**知恵の評議会 full 合議（10体の評価者）** が本エンジンを評価し、分類 **discovery_target**
（現在価値 54.5 / 潜在価値 60.3）を得た。評価者全員が「装置の完成度は名手級、効能（統合優位）は
未実証」で一致している。
...
| 軸 | 改訂 |
|---|---|
| ①統合優位性の実証 | `compare --runs N` の集計に勝率 95%CI（Wilson）・差の t 信頼区間・効果量（Cohen's d）を追加。n≥10 の実測で立証する装置へ |
| ②発散の感情的・意味的真実の残存 | `evaluation/specificity.py` の**具体性保存指数**で、草案の固有名詞・数字・独自造語が統合後どれだけ残るかを実測 |
...
├── render_comparison.py        # compare 出力から「素AI生成 vs 統合版」比較ドキュメントを生成
├── requirements.txt
└── README.md
```
