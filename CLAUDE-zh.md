...
- **三契机辩证法**: 否定・保存・高次化这三契机是机构的核心。通过 创意水平（`--idea-level`）选择 发散 与 Aufheben 要走到多极端：standard 0.9（默认・向后兼容）/ very 1.2 / extreme 1.5。每个水平配对一个递进式发散提示。最终化的温度 0.0 固定。
- **命题集中形式**: 草案由 核心主张 / 依据 / 前提 这 3 个要素构成・500〜800 字。DRAFT_MAX_LENGTH=1000（创作类放宽到 3000）。
- **i18n 基准**: 多语言支持（en/ja/zh）是任何修改・变更的默认基准。新增或修改的 prompt / CLI 消息 / 模板 / 保存输出必须通过三层机制（locale JSON / 按语言 prompt / 镜像树）解析，面向用户的文本用运行语言（`--lang`）生成。

## 测试

用 `pytest tests/` 确认全部。259 件（截至 2026-08-11）。
