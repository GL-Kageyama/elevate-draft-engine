...
| 4 | キャッチコピー | リサイクル素材のスニーカー『Maru』のキャッチコピーを開発せよ | 材料・ターゲット・価格帯・ブランド価値 | メイン「海のゴミが、走り続ける。」＋サブ「あなたの一歩で、海へ帰る。Maru。」。再生PET100%・-70%削減・海を守る一足を捏造なく織り込み、-70%は明示せず円環で担う（分析レポート化せず短形式の成果物に）。抽出 min=150/max=800・output_is_direct | 0 |
| 5 | プレスリリース | リサイクル素材スニーカー『Maru』の新製品発表プレスリリースを作成せよ | 会社・製品・発売日・価格・特長 | 見出し「海から帰って、まだ回っている。」。配信日/報道関係各位/リード/担当者コメント/会社概要/問い合わせ先の標準構成。Maru・港区・10月1日・12,000円・70%削減・軽量通気性を捏造なく織り込み、「残り30%は未達成」を明示（誠実性）。抽出 min=600/max=2500・output_is_direct | 0 |
| 6 | 研究アブストラクト | 量子アニーリングの実用化に関する研究アブストラクトを書け | 分野・現状・課題・応用・制約 | タイトル/背景/目的・方法/結果/結論/キーワードの学術構成。組合せ最適化・物流/創薬/ポートフォリオ・数千量子ビット級実機・ノイズ/スケール/誤差補正/問題マッピングを織り込み、実用化を「解くこと」ではなく「風景の再記述」と再定義（問題写像の四契機）。抽出 min=600/max=1500・output_is_direct | 0 |
