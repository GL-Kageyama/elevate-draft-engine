"""improve（統合版 → 改修の草案(複数) → 統合 の反復改善ループ）のテスト。

検証対象: main.py（cmd_improve / _revision_task / _save_progress）
- ループ配線: round 2 以降は前回の統合版を改修対象に改修草案を作り、それを統合する
- round_NN/ ごとの成果物保存
- --evaluate の頭打ち早期停止。mock はポリシー密着5軸（2026-08-08 再調整(3)）後、
  素の生成相当（round 1）0.600 → 統合版 0.720 と向上が可視化され、改修度3で頭打ち → 決定的に停止する
...
def test_improve_second_round_inherits_previous_elevated(tmp_path, monkeypatch) -> None:
    """round 2 の改修草案タスクに round 1 の統合版が埋め込まれる。
    ...
    # round 2 の改修草案コールに「改修対象: 前回の統合版」と round 1 の統合版が入る
    draft_users = [user for (system, user, _) in gen.calls if "草案の作り方" in system]
    assert any("改修対象: 前回の統合版" in u for u in draft_users)
...
def test_improve_first_round_uses_original_task(tmp_path, monkeypatch) -> None:
    ...
    assert not any("改修対象: 前回の統合版" in u for u in draft_users[:8])
    # 後半（round 2）の草案コールにはある
    assert any("改修対象: 前回の統合版" in u for u in draft_users[8:])


# ---- round ごとの成果物保存 ----

def test_improve_rounds_save_each_round(tmp_path) -> None:
    """rounds>1 なら round_NN/ ごとに発散・推理・統合・進捗が保存される。"""
    code, _ = _run_improve(
...
def test_improve_progress_marks_loop(tmp_path) -> None:
    """progress.md にループの説明（改修草案 → 統合）が記録される。"""
    code, _ = _run_improve(
        ["improve", "タスク", "--mock", "--rounds", "2", "--out", str(tmp_path)]
    )
    assert code == 0
...
def test_improve_visible_improvement_then_plateau(tmp_path) -> None:
    """mock で素の生成相当（round 1）0.600 → round 2 統合版 0.720 と向上が可視化され、頭打ちで早期停止する。

    ポリシー密着5軸（2026-08-08 再調整(3)）の下でも、素の生成相当は天井（0.79）でなく
    0.600 になり、統合版を磨くごとに overall が上昇する（向上が可視化できる）。
    改修度3で 0.720 に頭打ち → 停止。
    """
    ...
    assert "0.720" in text  # round 2 の統合版で Pass（0.720）に上昇
